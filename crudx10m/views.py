...
from .serializers import WorkClassSerializer, EducationSerializer, OccupationSerializer, MaritalStatusSerializer, NativeCountrySerializer, RaceSerializer, SexSerializer, RelationshipSerializer, DemographicDataSerializer
...
class DemographicDataAPIView(APIView):
    def get(self, request, pk=None):
        cache_key = f'demographicdata_{pk}' if pk else 'all_demographicdata'
        data = cache.get(cache_key)
        ...
        if not data:
            if pk:
                ...
                demographic_data = DemographicData.objects.all()
